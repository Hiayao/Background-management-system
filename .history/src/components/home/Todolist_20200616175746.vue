<template>
  <div class="todo_list">
    ...
    <div class="box">
      ...
      <div v-for="(item1,index1) in todos1" :key="index1" class="box1">
        <div @mouseleave="leave(item1)" @mouseenter="enter(item1)" class="mouse">
          <div>
            <input type="checkbox" v-model="item.complete" />
          </div>
          <div class="word">{{item1.title}}</div>
          <div>
            <i class="el-icon-close" v-if="item1.hover" @click="del(index1)"></i>
          </div>
        </div>
      </div>
    ...
    <div class="foot">
      ...
      <div class="footTwo">
        ...
        <div class="footTB" @click="border2(item1,index1)" :class="{'border':flag2 === true}">Active</div>
